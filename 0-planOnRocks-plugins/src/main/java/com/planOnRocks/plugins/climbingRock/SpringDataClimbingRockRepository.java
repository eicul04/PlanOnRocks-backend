...
public interface SpringDataClimbingRockRepository extends JpaRepository<ClimbingRock, Long> {
    ...
    @Query(value = "SELECT * FROM CLIMBING_ROCK where DIFFICULTY = 'EASY' AND BOLTING = 'GOOD' OR BOLTING = 'VERY_GOOD'", nativeQuery = true)
    List<ClimbingRock> getClimbingRocksForBeginnerParticipants();

    @Query(value = "SELECT * FROM CLIMBING_ROCK where DIFFICULTY = 'EASY' OR DIFFICULTY = 'MEDIUM' AND BOLTING = 'OK' OR BOLTING = 'GOOD' OR BOLTING = 'VERY_GOOD'", nativeQuery = true)
